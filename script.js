...
function aggiornaStoricoUI() {
    ...
    storicoIMC.forEach((item, idx) => {
        ...
        delBtn.addEventListener('click', () => {
            storicoIMC.splice(idx, 1);               // Rimuove dall'array
...
// gestione IMC
document.getElementById('calcolaIMC')?.addEventListener('click', () => {
...
// gestione fabbisogno calorico
document.getElementById('calcolaFB')?.addEventListener('click', () => {
...
function aggiornaDiarioUI() {
    ...
    if (!ul) return;
    // svuota la lista per non accumulare il contenuto
    while (ul.firstChild) {
...
// aggiungere attività
document.getElementById('btnAggiungiAttivita')?.addEventListener('click', () => {
